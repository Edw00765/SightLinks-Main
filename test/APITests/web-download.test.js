...
describe('Web Download Endpoint', () => {
  describe('GET /download/:token', () => {
    it('downloads ZIP file when detections are found', async () => {
      ...
      let attempts = 0;
      const maxAttempts = 48; // Increased to 4 minutes total
      ...
      // Add a delay before attempting download to ensure file is ready
      await new Promise(resolve => setTimeout(resolve, 5000));

      // Now test the download
      const downloadResponse = await fetch(`${BASE_URL}/download/${downloadToken}`);
    ...
    it('downloads text file when no detections are found', async () => {
      // Use no-crossings dataset which has no detections
      const noCrossingsPath = path.join('data-and-images', 'digimap-data', 'no-crossings.zip');
      ...
      expect(downloadResponse.headers.get('Content-Disposition')).toMatch(/attachment; filename=result_\d{8}\.txt/);
      expect(downloadResponse.headers.get('X-Has-Detections')).toBe('false');
    ...
    it('handles empty ZIP files appropriately', async () => {
      ...
      // Try to download the empty ZIP file
      const downloadResponse = await fetch(`${BASE_URL}/download/${downloadToken}`);
      
      // The server treats empty ZIPs as "no detections" cases
      expect(downloadResponse.status).toBe(200);
      expect(downloadResponse.headers.get('Content-Type')).toContain('text/plain');
      expect(downloadResponse.headers.get('X-Has-Detections')).toBe('false');
      
      const responseText = await downloadResponse.text();
      expect(responseText).toBe('No detections were found in any of the images.');
